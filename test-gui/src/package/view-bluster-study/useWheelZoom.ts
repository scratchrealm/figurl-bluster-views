...
import { AffineTransform, applyAffineTransform, createAffineTransform, identityAffineTransform, inverseAffineTransform, multAffineTransforms } from "../component-scene2d/AffineTransform"

const useWheelZoom = (x: number, y: number, width: number, height: number, opts: {shiftKey: boolean}) => {
    ...
    const handleWheel = useCallback((e: React.WheelEvent) => {
        if ((opts.shiftKey) && (!e.shiftKey)) return
        ...
        return false
    }, [affineTransform, x, y, height, width, opts])
    return {
        affineTransform,
        handleWheel
    }
